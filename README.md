...
## Recipe catalog boundaries

The recipe tree permits only `en`, `fr`, and `ru` directories containing regular
files named exactly `<slug>.json` with NFC-normalized raw Unicode. Bounded
strict-JSON loading rejects malformed UTF-8, duplicate keys, excessive nesting,
oversized values, and catalog overflow. Source-backed WordPress recipe URLs
remain in `redirectFrom`; raw editorial HTML remains provenance.

Loading checks path and descriptor identities and revalidates the root and
locale tree. Node does not expose portable `openat` traversal, so a hostile
local process that swaps and restores ancestor directories entirely between
checks remains outside this boundary. Builds require a trusted local workspace.
